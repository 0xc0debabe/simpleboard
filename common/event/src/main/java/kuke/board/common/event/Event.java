...
public class Event <T extends EventPayload> {
    ...
    public static Event<EventPayload> fromJson(String json) {
        EventRaw eventRaw = DataSerializer.deserializer(json, EventRaw.class);
        if (eventRaw == null) return null;

        Event<EventPayload> event = new Event<>();
        event.eventId = eventRaw.getEventId();
        event.type = EventType.from(eventRaw.getType());
        event.payload = DataSerializer.deserializer(eventRaw.getPayload(), event.type.getPayloadClass());
        return event;
    }
